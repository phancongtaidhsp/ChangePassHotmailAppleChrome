...
const action = async (browser, context, page, record) => {
  ...
  await page.goto('https://outlook.live.com/mail/0/options/mail/forwarding', { timeout: 90000 });

  await page.waitForSelector('.fui-DialogActions button:nth-child(2)', { timeout: 90000 });

  await delay(3000);

  await page.click('.fui-DialogActions button:nth-child(2)');

  await page.waitForSelector('.ms-Modal-scrollableContent div[role="tabpanel"] .customScrollBar button', { timeout: 90000 });
  ...
  await popup.waitForSelector('input[type="password"]', { timeout: 90000 });
  ...
  await popup.waitForSelector('#EmailAddress', { timeout: 90000 });
  ...
  await pageTempEmail.goto('https://smvmail.com', { timeout: 90000 });
  await pageTempEmail.waitForSelector('input[name="email"]', { timeout: 90000 });
  await delay(2000);
  await pageTempEmail.type('input[name="email"]', mailTemp, { delay: 30 });
  await delay(2000);
  await pageTempEmail.click('form button');
  await delay(2000);
  await pageTempEmail.waitForSelector('header button', { timeout: 90000 });
  ...
  await popup.waitForSelector('#iOttText', { timeout: 90000 });
  await delay(2000);
  await pageTempEmail.bringToFront();
  await delay(2000);
  let codeMailTemp = '';
  for (let i = 0; i < 7; i++) {
    ...
    if (aElment) {
      ...
      await pageTempEmail.waitForSelector('tbody', { timeout: 90000 });
      await delay(3000);
  ...
  await popup.click('.position-buttons input[type="submit"]');
  await delay(2000);
  await popup.waitForSelector('#view button:nth-child(2)', { timeout: 90000 });
  await delay(2000);
